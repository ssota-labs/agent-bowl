...
미리보기 창이 **사용자가 보는 유일한 화면**이다. 이제는 **시각 편집기**다.

- 슬라이드 영역을 **클릭**하면 우측 패널에서 글·색·크기 등을 고칠 수 있다. 저장은 원본 `slides/*.html`에 바로 반영된다
- 왼쪽에서 슬라이드 / 템플릿 / 팔레트 탭을 고른다. 팔레트는 `assets/theme.css` 토큰을 고친다
- 에이전트가 파일을 고쳐도 **0.6초 안에** 화면이 갱신된다. 반대로 사용자가 UI로 고친 것도 같은 파일이다
...
> 미리보기(편집기) 창을 띄웠습니다. 이 창은 **계속 켜두시면 됩니다** — 제가 슬라이드를 고칠 때마다
> 알아서 바뀝니다. 새로고침하실 필요 없어요.
>
> - 화면에서 **영역을 클릭**해 글·색을 직접 고칠 수 있습니다. 말로 알려주셔도 됩니다.
> - 왼쪽 **팔레트**에서 덱 색을, **템플릿**에서 새 장표 원형을 관리합니다.
...
│   ├── theme.css      ★ 팔레트·폰트 — 덱마다 여기만 고친다 (/* slidecraft:tokens */)
...
├── templates/*.html   장표 원형 — 편집기에서 고치고 새 슬라이드로 복제
